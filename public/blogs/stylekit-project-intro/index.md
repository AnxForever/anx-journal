# StyleKit：把 UI 风格做成一个能被 AI 调用的设计系统

最近我在持续做一个项目，叫 **StyleKit**。

它不是单纯的样式展示站，也不只是一个“灵感收集库”。我真正想做的，是把 UI 风格本身整理成一种可以被人和 AI 共同使用的结构化资产。

## 它是什么

简单说，StyleKit 是一个围绕设计风格构建的平台：

- 可以浏览和筛选大量 UI 风格
- 可以查看模板、动画和设计资源
- 可以拿到 Prompt、设计 token 和组件 recipes
- 可以通过 API、CLI、MCP 等方式接入开发工作流

也就是说，它不是停留在“看图找灵感”这一层，而是在尝试把“风格”变成一种真正可消费、可复用、可集成的能力。

## 我为什么想做这个

现在很多设计灵感网站有两个问题：

1. 好看，但停留在浏览层
2. 有参考价值，但很难直接进入实际开发链路

而 AI 编码工具出现之后，这个问题反而变得更明显了。

我们不缺“UI 看起来像什么”的案例，缺的是一套能告诉 AI 和开发者：

- 这个风格的核心气质是什么
- 颜色、排版、边框、阴影应该怎么落
- 按钮、卡片、输入框这些组件应该往哪里靠
- 什么是这个风格应该做的，什么又最好不要做

StyleKit 想解决的，就是这个中间层。

## 它现在在做哪些事

目前这项目已经不只是一个网站首页了，它更像一个多出口的平台：

- 网站前台：浏览 styles、templates、animations 和各种设计资源
- API：把风格数据和校验能力开放出去
- CLI：在本地工作流里直接调用规则和导出能力
- MCP Server：让 AI assistant 可以直接读取风格系统
- Core package：把 styles、recipes、linter、knowledge 等能力做成可复用包

对我来说，这也是它最有意思的地方。

我不是在做一个“展示型产品”，而是在尝试把设计知识本身做成系统。

## 它适合谁

我觉得它最适合这几类人：

- 想快速确定界面方向的独立开发者
- 需要给 AI 更稳定设计约束的前端工程师
- 想把风格沉淀成内部规范的小团队
- 不想从零搭设计语言，但又不满足于直接复制别人界面的产品人

如果你经常遇到“AI 写出来的页面能用，但没气质”这种问题，那你大概就会理解我为什么要做它。

## 我希望它以后长成什么样

后面我会继续把它往几个方向推：

- 更完整的 style catalog
- 更稳定的导出和规则系统
- 更顺手的 AI prompt 工作流
- 更清晰的社区投稿和审核机制

我希望它最后不是“一个我自己觉得还不错的网站”，而是一套别人也能真正拿去使用的工具。

## 你可以去看看

- 官网：<https://stylekit.top>
- 项目地址：<https://github.com/AnxForever/stylekit>

如果你也在关注 AI 编码、设计系统、Prompt 工作流，或者只是想找更有质感的 UI 方向，StyleKit 也许会对你有用。
